...
export const etapaService = {
  ...
  buscarPorId: (id) => api.get(`/etapas/${id}`),
  listarPorAeronave: (aeronaveId) => api.get(`/etapas/aeronave/${aeronaveId}`),
  listarPorStatus: (status) => api.get(`/etapas/status/${status}`),
  criar: (data) => api.post('/etapas', data),
  atualizar: (id, data) => api.put(`/etapas/${id}`, data),
  deletar: (id) => api.delete(`/etapas/${id}`),
  associarFuncionario: (etapaId, funcionarioId) => 
    api.post(`/etapas/${etapaId}/funcionarios/${funcionarioId}`),
  desassociarFuncionario: (etapaId, funcionarioId) => 
    api.delete(`/etapas/${etapaId}/funcionarios/${funcionarioId}`),
};
...
export const relatorioService = {
  ...
  completo: () => api.get('/relatorios/completo'),
};
